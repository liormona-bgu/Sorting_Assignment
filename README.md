...
We generated random integer arrays. We tested the sizes: 100, 500, 1000, 5000, 7500, 10000, and repeated the experiment for each size 5 times using different arrays (the same size). 
...
The difference in growth rates between algorithms
The consistency (or variability) of their performance
Key Observations (optional – כדאי להוסיף אם רוצים ציון גבוה)
Merge Sort performs significantly better for large inputs (O(n log n))
Insertion Sort and Bubble Sort show slower growth (O(n²))
Variance increases with input size, especially for slower algorithms
